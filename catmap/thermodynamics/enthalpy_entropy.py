...
class ThermoCorrections(ReactionModelWrapper):
    ...
    def ideal_gas(self):
        ...
        for gas in gas_names:
            ...
            if gas in ['pe_g', 'ele_g', 'H_g', 'OH_g',
                       self.proton_species, self.hydroxide_species]:  # TODO: doc
                thermo_dict[gas] = 0.
        ...
        for gas in gas_names:
            if gas in shomate_params.keys():
                ...
                loc = [_ for _ in [_ if ((temperature >= params[_]['T_min'] and temperature <= params[_]['T_max']) or \
                                         (temperature < params[_]['T_min'] and params[_]['T_min'] < 300)) else None for
                                   _ in range(len(params))] if _ is not None]
                if len(loc) > 0:
                    params = params[loc[0]]
                    ...
                    raise ValueError(
                        'No Shomate parameters available for T = {}  specified for {}'.format(temperature, gas))
            else:
    ...
    def shomate_adsorbate(self):
        ...
        for ads in adsorbate_names:
            ...
            if ads in shomate_params:
                ...
                loc = list(filter(None.__ne__,
                                  [_ if ((temperature >= params[_]['T_min'] and temperature <= params[_]['T_max']) or \
                                         (temperature < params[_]['T_min'] and params[_]['T_min'] < 300)) else None for
                                   _ in range(len(params))]))
                if len(loc) > 0:
                    if self.frequency_dict[ads]:
                        ZPE = sum(self.frequency_dict[ads]) / 2.0
                        self._zpe_dict[ads] = ZPE
                    else:
                        self.average_transition_state(thermo_dict, transition_state_list=[ads],
                                                      thermo_vars=[self._zpe_dict])
                    ...
                    raise ValueError(
                        'No Shomate parameters available for T = {}  specified for {}'.format(temperature, ads))
            elif '-' in ads:
                avg_TS.append(ads)
            else:
    ...
    def get_pressure_equilibrium(self, xguess=None, ftol=1e-5):
        ...
        self.gas_pressures = np.array([self.pressure * _ / sum(self.gas_pressures) for _ in self.gas_pressures],
                                      dtype=np.float128)
        ...
        energies = np.array([sps_dict[_]['formation_energy'] + thermo_corrections[_] for _ in gas_species],
                            dtype=np.float128)
        ...
        jac_total_gibbs = lambda x: np.array(energies / float(self.kBT) + np.log(x + 1e-300) + np.ones(len(x)),
                                             dtype=np.float128)
        ...
        xeq = minimize(total_gibbs, xguess if xguess else x0, method='SLSQP', jac=jac_total_gibbs,
                       ...
                       options={'ftol': ftol, 'disp': False, 'maxiter': 1000, 'eps': 1.4901161193847656e-08},
                       bounds=bounds)
    ...
    def set_equilibrated(self):
        ...
        """
        return dict(zip(('xeq', 'xf', 'x0', 'Aeq', 'beq', 'Aeq2', 'beq2', 'Geq', 'Gf', 'G0'),
                        (xeq, xf.x, x0, Aeq, beq, Aeq2, beq2, Geq, total_gibbs(xf.x), G0)))
...
def fit_shomate(Ts, Cps, Hs, Ss, params0=[], plot_file=None):
    ...
    if plot_file:
        import pylab as plt
        print('R2: {}, sumE: {}'.format(np.corrcoef(M.dot([A, B, C, D, E, F, G]), b),
                                        (M.dot([A, B, C, D, E, F, G]) - b).T.dot(M.dot([A, B, C, D, E, F, G]) - b)))
        fig = plt.figure()
