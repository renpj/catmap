...
class MinResidMapper(MapperBase):
    ...
    def get_initial_coverage_from_map(self, descriptors, *args, **kwargs):
        ...
        """
        resid_cvg = []  # store the largest coverage rate for each point
    ...
    def get_coverage_map(self, descriptor_ranges=None, resolution=None,
        ...
        d1Vals = d1Vals[::-1]  # reverse the array
        d2Vals = d2Vals[::-1]

        # isMapped is a Matrix to track which values have been checked
        # or which directions have been searched
        ...
        def minresid_iteration(isMapped):
            ...
            for i in range(0, m):
                for j in range(0, n):
                    ...
                    if isMapped[i, j] < maxNum:  # the point has not been find yet
                        # Get the list of possible guess coverages based
                        # on the search directions
                        checked_dirs = [int(bi) for bi in np.binary_repr(isMapped[i, j])]
